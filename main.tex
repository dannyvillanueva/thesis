...
\section{Building the Dataset for Training}
...
\begin{itemize}
	...
	\item  $M_{T_p}$ - classification of the tweet $T_p$: 0 - not related with a medical condition, 1 - related with a medical condition, 2 - ambiguous. 
	...
	\item  $M_{T_{h_1}}$ - classification of the tweet  $T_{h_1}$: 0 - not related with a medical condition, 1 - related with a medical condition, 2 - ambiguous. 
	...
	\item  $M_{T_{h_2}}$ - classification of the tweet $T_{h_2}$: 0 - not related with a medical condition, 1 - related with a medical condition, 2 - ambiguous. 
	...
	\item $L$ - relative comparison label : 1 - if $rank(T_p, T_{h_1}) >= rank(T_p, T_{h_2})$, 0 - if  $rank(T_p, T_{h_1}) < rank(T_p, T_{h_2})$
...
The tuples described in the previous section are used in our method, and we train a model $M'$ on a classification task: determine which tweet is more similar to $T_p$ : 
$T_{h_1}$ or $T_{h_2}$. In doing so, we will be training a sub-model $M$ that does a regression tasks: compute the $rank(T_p, T_{h_i})$ between the 
premise and a hypothesis. $M$ will become the model that is actually used in production to rank tweets. 
Our approach  is based on a triplet-based network architecture \cite {Wang2014} to classify if tweets are similar or not. It is based in the level of similarity relevance between tweets and their relatedness with a disease  The inputs to train the algorithm are given in \ac{CSV} file that contains tweets and the label values (see section \ref{data_labeling}). Input data passes through a process to finally get the measures of similarity between a pair of tweets. The entire process is shown in Figure \ref{figure:all_model}
...
The proposed model starts with an embedding layer. Each of the tweets in a tuple passes through an embedding process to convert sentences into a matrix of multidimensional vectors, each row representing words. Next, in a combiner layer we use different approaches such \ac{RNN} and \ac{CNN} to extract 
features from the data.
We use  several combinations of hyperparameters until get better results.
In the relevance layer we add a second input, called auxiliary input, that contains the information about the medical keyword of the tweet and the label indicating if the tweet is actually related with the medical condition or not. 
 Using auxiliary input and the output of combiner layers we calculate the relevance between the premise tweet and hypothesis tweet. This value become the 
 rank of the premise and hypothesis. Up to this level, the neural network has trained $M$, the model that computes similarity ranking and the one we
  actually want in production. But  the process
 is not finished.
The final network component is the classification layer that is composed of a sequence of dense layers to finally get an output with values of 1 or 0, where 1 means that the first hypothesis is more similar to the premise, and 0 means that the second hypothesis is more similar to the premise. This neural network is model $M'$. Notice how 
training $M'$ indirectly trains model $M$.
